...
export function StepMetal({ value, onChange, onAutoAdvance }: Props) {
  const options = DESIGNER_OPTIONS.metal;
  ...
  return (
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-16">
      ...
      <p className="font-accent text-content-secondary font-light mb-10 text-center">
        El material que dará vida a tu diseño
      </p>
      <div className="w-full max-w-3xl grid grid-cols-2 lg:grid-cols-4 gap-4">
        {options.map((opt) => (
          <SelectionCard
            key={opt.id}
            id={opt.id}
